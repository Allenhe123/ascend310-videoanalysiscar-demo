...
int main(int argc, char* argv[])
{
    ...
    // 1.create graph
    // 每个graph是一个进程，每个Engine是1个（一般为1个，也可启动多个）线程，进程可以配置优先级（config文件内配置）。
    // HIAI_InitAndStartGraph()函数会读取配置文件
    // 创建和启动各个线程（线程无优先级配置选项）。各个Engine被编译成1个或多个so文件。
    ...
    // 消息名称为string，用一个空消息作为数据输入来驱动整个个流水线工作
